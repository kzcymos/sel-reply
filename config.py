...
def get_params():
    ...
    parser.add_argument("--memory_buffer_size", type=int, default=1000, 
                        help="记忆缓冲总大小（所有旧任务样本的总和）")
    parser.add_argument("--replay_kmeans_clusters", type=int, default=5, 
                        help="K-Means 聚类数（每个旧实体类型的簇数）")
    parser.add_argument("--replay_samples_per_entity", type=int, default=20, 
                        help="每个旧实体类型选择的代表样本数")
    parser.add_argument("--replay_weight", type=float, default=1.0, 
                        help="重放损失的权重（类似于 distill_weight）")
    parser.add_argument("--replay_adaptive_factor", type=bool, default=True, 
                        help="是否自适应调整重放损失权重（类似于 classif_adaptive_factor）")
                        
    
    parser.add_argument("--temporal_decay_factor", type=float, default=0.5, 
                        help="重放损失的权重（类似于 distill_weight）")
    # Debug
