...
def main_cl(params):
    ...
    for iteration in range(total_iter): # 遍历每个任务
        ...
        # Update the architecture of the classifier
            ...
            #copy_odd_layers(trainer.model, trainer.refer_model, params.model_name) # 只复制奇数层，偶数层用预训练权重
            ...
            trainer.enable_selective_distillation(old_dataloader)  # 传入旧任务数据，神经选择
            trainer.refer_model = deepcopy(trainer.model) # old model
            #copy_odd_layers(trainer.model, trainer.refer_model, params.model_name) # 只复制奇数层，偶数层用预训练权重
            trainer.refer_model.eval()
            # Change model classifier
        ...
        if iteration==0:
            ...
            old_entity_list = ner_dataloader.entity_list[\
                                :params.nb_class_fg + (iteration - 1) * params.nb_class_pg]# 旧任务的实体集合
  
        # 计算旧实体嵌入（仅iteration > 0）
        if iteration > 0:
            ...
            old_dataloader = old_dataloader_tuple[0]
            trainer.compute_entity_embeddings(old_entity_list, old_dataloader, ner_dataloader)

        # ... (继续原有训练代码：分类器更新、优化器设置、训练循环等)
        ...
        if iteration>0:
            ...
            memory_dataloader = select_memory_samples(ner_dataloader, trainer, old_entity_list, params)
            trainer.memory_dataloader = memory_dataloader  # 存到 trainer
        else:
        ...
        for e in range(1, training_epochs+1):
            ...
            loss_list, distill_list, ce_list,replay_list,fnn_list= [], [], [], [],[]
            # average loss
            mean_loss = 0.0
            # training acc
            ...
            for X, y in dataloader_train:
                ...
                # Compute loss
                if iteration>0:
                    ce_loss, distill_loss, replay_loss,fnn_loss= trainer.batch_loss_cpfd(y,dataloader_test_cumul,all_seen_entity_list,old_classes_list, new_classes_list,new_entity_list)
                    ce_list.append(ce_loss)
                    distill_list.append(distill_loss)
                    replay_list.append(replay_loss)
                    fnn_list.append(fnn_loss)
                ...
                mean_replay_loss = np.mean(replay_list) if len(replay_list)>0 else 0
                mean_fnn_loss = np.mean(fnn_list) if len(fnn_list)>0 else 0
           

                # Print training information
                if params.info_per_steps>0 and step%params.info_per_steps==0: # params.info_per_steps=0
                    logger.info("Epoch %d, Step %d: Total_loss=%.3f, CE_loss=%.3f, Distill_loss=%.3f, replay_loss=%.3f,Training_exact_match=%.2f%%"%(
                            e, step, mean_loss, \
                            mean_ce_loss, mean_distill_loss, mean_replay_loss, correct_cnt/total_cnt*100
                    ))
                    # reset the loss lst
                    loss_list = []
                    distill_list = []
       ...
       # testing
        ...
        # 任务结束后，将当前分类器存入池中
        trainer.classifier_pool.append(deepcopy(trainer.model.classifier))
        logger.info(f"Classifier pool updated. Current size: {len(trainer.classifier_pool)}")
#
#        
