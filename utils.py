...
from src.dataloader import NER_dataloader
...
def compute_task_similarity(old_entity_embs, new_entity_embs):
    """
    计算旧任务实体嵌入与新任务实体嵌入的语义相似性。
    - old_entity_embs: 旧任务实体嵌入列表 (list of tensors, each shape: [emb_dim])
    - new_entity_embs: 新任务实体嵌入列表 (list of tensors, each shape: [emb_dim])
    返回: 平均余弦相似性分数 (scalar)
    """
    old_embs = torch.stack(old_entity_embs).cpu().numpy() if isinstance(old_entity_embs[0], torch.Tensor) else np.array(old_entity_embs)
    new_embs = torch.stack(new_entity_embs).cpu().numpy() if isinstance(new_entity_embs[0], torch.Tensor) else np.array(new_entity_embs)
    sim_matrix = cosine_similarity(old_embs, new_embs)
    return np.mean(sim_matrix)  # 平均相似性作为任务相关性分数

def compute_temporal_decay(task_ages, decay_factor=0.5):
    """
    计算时间衰减权重。
    - task_ages: 任务年龄列表 (e.g., [0,1,2,...] 表示任务新近度，越大越旧)
    - decay_factor: 衰减因子 (0<decay_factor<1，越小衰减越快)
    返回: 衰减权重 tensor
    """
    ages = torch.tensor(task_ages, dtype=torch.float32).cuda()
    decay_weights = torch.exp(-decay_factor * ages)  # 指数衰减
    return decay_weights / decay_weights.sum()  # 归一化
# 新增函数：复制奇数层并用预训练权重初始化偶数层
def copy_odd_layers(source_model, target_model, pretrained_model_name):
    """
    从 source_model 复制 BERT 的奇数层到 target_model，偶数层从预训练权重初始化。
    
    参数:
        - source_model: 源模型（学生模型，包含当前训练的权重）
        - target_model: 目标模型（教师模型，将被修改）
        - pretrained_model_name: 预训练模型名称（如 'bert-base-cased'），用于初始化偶数层
    """
    source_layers = source_model.encoder.bert.encoder.layer  # ModuleList of 12 BertLayer
    target_layers = target_model.encoder.bert.encoder.layer  # 目标模型的 layer（也是12层）
    
    # 加载预训练模型以获取初始权重
    pretrained_model = AutoModelWithLMHead.from_pretrained(pretrained_model_name)
    pretrained_layers = pretrained_model.bert.encoder.layer  # 预训练的 layer
    
    odd_indices = [0,1, 3, 5, 7, 9,11]  # 0索引的奇数层：1,3,5,7,9,11
    even_indices = [ 2, 4, 6, 8,10]  # 0索引的偶数层：0,2,4,6,8,10
    
    # 确保层数一致
    assert len(source_layers) == len(target_layers) == len(pretrained_layers) == 12, \
        "Layer count mismatch between source, target, or pretrained model!"
    
    # 复制奇数层权重
    for i in odd_indices:
        target_layers[i].load_state_dict(deepcopy(source_layers[i].state_dict()))
    
    # 为偶数层加载预训练权重
#    for i in even_indices:
#        target_layers[i].load_state_dict(deepcopy(pretrained_layers[i].state_dict()))
    # 将偶数层初始化为0
    for i in even_indices:
        for param in target_layers[i].parameters():
            param.data.zero_()  # 将参数张量设为全零
    
    logger.info(f"Copied odd layers {odd_indices} from source model and initialized even layers {even_indices} from pretrained {pretrained_model_name}")

def select_memory_samples(ner_dataloader, trainer, old_entity_list, params):
    memory_buffer = []  # 存储 (X, y) 元组的列表
    
    # 获取模型所在的设备
    model_device = next(trainer.model.parameters()).device
    logger.info(f"模型设备: {model_device}")
    
    # 获取旧任务的训练数据（所有旧实体的训练数据，不过滤 O）
    old_dataloader_tuple = ner_dataloader.get_dataloader(
        first_N_classes=len(old_entity_list),
        select_entity_list=old_entity_list,
        phase=['train'],
        is_filter_O=False,
        reserved_ratio=1.0  # 使用所有旧数据选择样本
    )
    old_dataloader = old_dataloader_tuple[0]  # 提取 tuple 的第一个元素（train DataLoader）
    
    for entity in old_entity_list:
        logger.info(f"处理实体: {entity}")
        # 过滤只包含该实体的样本
        entity_features = []
        entity_samples = []  # 该实体的 (X, y) 样本（句子级）
        sample_token_counts = []  # 每个样本的实体 token 数，用于映射
        for X_batch, y_batch in old_dataloader:
            # 确保批量数据移动到模型所在的设备
            X_batch = X_batch.to(model_device)
            y_batch = y_batch.to(model_device)
            #logger.debug(f"批量 X_batch 设备: {X_batch.device}, y_batch 设备: {y_batch.device}")
            
            with torch.no_grad():
                features = trainer.model.forward_encoder(X_batch)  # (bsz, seq_len, hidden_dim)
                # 提取包含该实体的 token 特征（mask 出实体位置）
                entity_mask = (y_batch == ner_dataloader.label_list.index(f'B-{entity}')) | \
                              (y_batch == ner_dataloader.label_list.index(f'I-{entity}'))
                num_entity_tokens = entity_mask.sum().item()
                if num_entity_tokens > 0:
                    entity_features.append(features[entity_mask].cpu().numpy())  # 收集实体 token 的特征
                    # 添加包含该实体的样本，并记录每个样本的实体 token 数
                    for i in range(X_batch.size(0)):
                        sample_token_count = entity_mask[i].sum().item()
                        if sample_token_count > 0:
                            # 存储 CPU 张量以节省 GPU 内存
                            entity_samples.append((X_batch[i].cpu(), y_batch[i].cpu()))
                            sample_token_counts.append(sample_token_count)
        
        if len(entity_features) == 0:
            logger.warning(f"实体 {entity} 未找到样本")
            continue
        
        entity_features = np.concatenate(entity_features, axis=0)  # (total_entity_tokens, hidden_dim)
        logger.info(f"实体 {entity}: {len(entity_samples)} 个样本, {entity_features.shape[0]} 个实体 token")
        
        # K-Means 聚类
        kmeans = KMeans(n_clusters=params.replay_kmeans_clusters, random_state=params.seed)
        kmeans.fit(entity_features)
        distances = np.linalg.norm(entity_features - kmeans.cluster_centers_[kmeans.labels_], axis=1)
        
        # 选择每个簇中最接近中心的 token_idx
        selected_token_indices = []
        for cluster in range(params.replay_kmeans_clusters):
            cluster_indices = np.where(kmeans.labels_ == cluster)[0]
            if len(cluster_indices) > 0:
                closest_token_idx = cluster_indices[np.argmin(distances[cluster_indices])]
                selected_token_indices.append(closest_token_idx)
            else:
                logger.warning(f"实体 {entity} 的簇 {cluster} 为空")
        
        if not selected_token_indices:
            logger.warning(f"实体 {entity} 没有有效簇")
            continue
        
        # 映射 token_idx 到 sample_idx
        cumulative_tokens = np.cumsum(sample_token_counts)  # 累积 token 数：[t1, t1+t2, ...]
        selected_sample_indices = []
        for token_idx in selected_token_indices:
            # 找到 token_idx 所属的样本（使用 np.searchsorted）
            sample_idx = np.searchsorted(cumulative_tokens, token_idx + 1)  # +1 因为 searchsorted 返回插入点
            if sample_idx < len(entity_samples):
                selected_sample_indices.append(sample_idx)
            else:
                logger.warning(f"Token 索引 {token_idx} 超出累积 token 范围")
        
        # 去重并限制数量（避免重复样本）
        unique_sample_indices = list(set(selected_sample_indices))[:params.replay_samples_per_entity]
        if len(unique_sample_indices) < params.replay_samples_per_entity:
            logger.info(f"实体 {entity} 只有 {len(unique_sample_indices)} 个唯一样本（请求 {params.replay_samples_per_entity} 个）")
        
        selected_samples = [entity_samples[idx] for idx in unique_sample_indices]
        memory_buffer.extend(selected_samples)
    
    # 限制记忆缓冲总大小（如果超过，随机采样）
    if len(memory_buffer) > params.memory_buffer_size:
        memory_buffer = random.sample(memory_buffer, params.memory_buffer_size)
    
    logger.info(f"创建记忆缓冲区，包含 {len(memory_buffer)} 个样本")
    
    # 转换为 DataLoader，确保使用 ner_dataloader 的 collate_fn
#    if not hasattr(ner_dataloader, 'collate_fn'):
#        logger.error("ner_dataloader 缺少 collate_fn 属性，请在 NER_dataloader 中定义")
#        raise AttributeError("ner_dataloader 缺少 collate_fn 属性")
    
    memory_dataloader = torch.utils.data.DataLoader(
        memory_buffer,
        batch_size=params.batch_size,
        shuffle=True,
        collate_fn=ner_dataloader.collate_fn  # 强制使用 NER_dataloader 的 collate_fn
    )
